...
# 2. تنسيق CSS: إزالة التكرار وضبط الجدول
st.markdown("""
    <style>
    ...
    /* الخط الجانبي وتعديل الدائرة لعدم التكرار */
    ...
    /* تنسيق الجدول */
    .styled-table { width: 100%; border-collapse: collapse; margin-top: 20px; background: white; border-radius: 10px; overflow: hidden; }
...
# --- صفحة التتبع بالجدول المنظم ---
    ...
    data = {
        "رقم الطلب": ["#1024", "#1025", "#1028"],
        "نوع الطلب": ["نقل داخلي", "تعديل مسمى", "تغيير قسم"],
        "تاريخ التقديم": ["2023-10-01", "2023-11-15", "2023-12-20"],
        "الحالة": ["مكتمل", "تحت المراجعة", "قيد الاعتماد"]
    }
    
    # رسم الجدول باستخدام HTML لتنسيق أجمل
    table_html = """
    <table class="styled-table">
        ...
        <tbody>
            <tr><td>#1028</td><td>تغيير قسم</td><td>2023-12-20</td><td><span class="status-badge status-pending">قيد الاعتماد</span></td></tr>
            <tr><td>#1025</td><td>تعديل مسمى</td><td>2023-11-15</td><td><span class="status-badge status-pending">تحت المراجعة</span></td></tr>
            <tr><td>#1024</td><td>نقل داخلي</td><td>2023-10-01</td><td><span class="status-badge status-done">مكتمل</span></td></tr>
        </tbody>
    </table>
    """
    st.markdown(table_html, unsafe_allow_html=True)
    
    if st.button("العودة للنموذج الرئيسي"):
        st.session_state.page = 'form'
        st.rerun()
