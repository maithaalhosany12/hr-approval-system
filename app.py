...
# 1. إعداد الصفحة والتنسيق (ثابت وأصيل كما طلبتِ)
...
st.markdown("""
    ...
    .attachment-box { background-color: #fff9e6; border: 1px dashed #ffd43b; padding: 10px; border-radius: 8px; margin-bottom: 15px; }
    </style>
    """, unsafe_allow_html=True)

# 2. إدارة حالة الصفحات (State Management)
...
# 3. الهيدر الرسمي
st.markdown('<div class="company-header"><div class="header-logo"><img src="https://cdn-icons-png.flaticon.com/512/281/281764.png"></div><div class="header-text"><h1>مؤسسة المسار المتكامل</h1><p>قسم الموارد البشرية - النموذج الموحد</p></div></div>', unsafe_allow_html=True)
...
if st.session_state.page == 'form':
    ...
    with st.container():
        st.markdown('<div class="content-box"><div class="step-header">📝 الخطوة الثانية: تفاصيل الطلب والمرفقات</div><div class="form-body">', unsafe_allow_html=True)
        c6, c7 = st.columns(2)
        ...
        if req_type in ["تعديل مهنة", "إنهاء خدمة"]:
            st.markdown(f'<div class="attachment-box">⚠️ يرجى إرفاق المستندات الداعمة لطلب {req_type}</div>', unsafe_allow_html=True)
            st.file_uploader("تحميل المرفق الرسمي", type=['pdf', 'png', 'jpg'])

        st.text_area("ملاحظات إضافية تفصيلية", height=100)
        c9, c10 = st.columns([3, 1])
        with c9: st.file_uploader("توقيعك الشخصي", type=['png', 'jpg'], key="sig")
        with c10: 
...
# --- الصفحة 2: متابعة الطلبات (الجدول المفقود) ---
    ...
    st.markdown("<h3 style='text-align:right;'>🔍 سجل المتابعة والطلبات</h3>", unsafe_allow_html=True)
    st.markdown(f"""
    <div class="content-box">
        <div class="form-body">
            <table class="styled-table">
                <thead>
                    <tr><th>رقم الطلب</th><th>نوع الطلب</th><th>التاريخ</th><th>الحالة الحالية</th></tr>
                </thead>
                <tbody>
                    <tr><td>{st.session_state.order_id}</td><td>طلب جديد</td><td>{datetime.now().strftime('%Y-%m-%d')}</td><td>بانتظار الاعتماد</td></tr>
                    <tr><td>REQ-9921</td><td>تعديل مهنة</td><td>2023-12-15</td><td>مكتمل</td></tr>
                </tbody>
            </table>
        </div>
    </div>
...
# --- الصفحة 3: الاعتمادات المتسلسلة (المفقودة) ---
    ...
    # ميزة اختيار الطلب
    order_pick = st.selectbox("اختر الطلب من السجل للمعالجة:", ["--- اختر طلباً ---", f"{st.session_state.order_id} - أحمد علي"])
    
    if order_pick != "--- اختر طلباً ---":
        st.info(f"أنت الآن بصدد مراجعة الطلب: {order_pick}")
        ...
        with col_m:
            ...
            m_d = st.selectbox("القرار ", ["قيد الانتظار", "موافق", "مرفوض"], key="m_d", disabled=not active)
            if active and st.button("اعتماد وإرسال لـ HR"):
                if m_d == "موافق": st.session_state.stage = 2; st.rerun()
        ...
        with col_hr:
            ...
            h_d = st.selectbox("القرار  ", ["قيد الانتظار", "موافق", "مرفوض"], key="h_d", disabled=not active)
            if active and st.button("اعتماد وإرسال للمدير العام"):
                if h_d == "موافق": st.session_state.stage = 3; st.rerun()
        ...
        with col_ceo:
            ...
            c_d = st.selectbox("القرار   ", ["قيد الانتظار", "موافق", "مرفوض"], key="c_d", disabled=not active)
            if active and st.button("إتمام الاعتماد النهائي"):
                if c_d == "موافق": st.session_state.stage = 4; st.balloons(); st.success("تم الاعتماد بنجاح")
