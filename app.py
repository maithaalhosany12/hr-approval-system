...
st.markdown("""
    ...
    .step-header { background: linear-gradient(90deg, #5d5fef, #7a7cfc); color: white; padding: 10px 25px; font-size: 15px; font-weight: bold; display: flex; justify-content: space-between; align-items: center; }
    .step-number-circle { background-color: rgba(255, 255, 255, 0.2); width: 26px; height: 26px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 13px; border: 1px solid white; }
...
if st.session_state.page == 'form':
    ...
    with st.container():
        st.markdown('<div class="content-box"><div class="step-header"><span>👤 الخطوة الأولى: بيانات مقدم الطلب</span><div class="step-number-circle">1</div></div><div class="form-body">', unsafe_allow_html=True)
        c1, c2, c3, c4, c5 = st.columns(5)
        with c1: job_id = st.text_input("الرقم الوظيفي")
        with c2: name = st.text_input("الاسم الكامل")
        with c3: job_title = st.text_input("المسمى")
        with c4: dept = st.text_input("القسم")
        with c5: join_date = st.date_input("تاريخ التعيين")
        st.markdown('</div></div></div>', unsafe_allow_html=True)

    # الخطوة 2
    st.markdown('<div class="step-block"><div class="step-icon">2</div>', unsafe_allow_html=True)
    with st.container():
        st.markdown('<div class="content-box"><div class="step-header"><span>📝 الخطوة الثانية: تفاصيل الطلب</span><div class="step-number-circle">2</div></div><div class="form-body">', unsafe_allow_html=True)
        ...
        with c9: sig = st.file_uploader("", type=['png', 'jpg'], label_visibility="collapsed")
        with c10: 
            st.markdown("<div style='height:0px;'></div>", unsafe_allow_html=True)
            submit_btn = st.button("إرسال الطلب", use_container_width=True)
        
        # منطق الإرسال المطور (أبطأ وأوضح)
        if submit_btn:
            if job_id and name and sig:
                # 1. إظهار رسالة انتظار
                msg_placeholder = st.empty()
                progress_bar = st.progress(0)
                
                for percent_complete in range(100):
                    time.sleep(0.02) # سرعة التحميل (إجمالي ثانيتين تقريباً)
                    progress_bar.progress(percent_complete + 1)
                
                progress_bar.empty()
                st.success("🎉 تم إرسال طلبك بنجاح! جاري تحويلك لصفحة المتابعة...")
                time.sleep(2) # تأخير إضافي لقرأة الرسالة
                st.session_state.page = 'tracking'
                st.rerun()
            else:
                st.error("⚠️ يرجى التأكد من تعبئة كافة الحقول ورفع التوقيع")
...
# --- صفحة التتبع ---
    ...
    st.success("✅ أنت الآن في صفحة تتبع الطلبات.")
    st.info("طلبك الأخير قيد المراجعة حالياً من قبل إدارة الموارد البشرية.")
