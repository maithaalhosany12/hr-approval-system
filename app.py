...
# --- التعديل المطلوب: إبقاء القائمة المنسدلة فقط بدون عناوين ---
with st.sidebar:
    # تم إزالة أسطر الـ markdown السابقة
    choice = st.selectbox(
        label="", # ترك العنوان فارغاً ليبقى فقط الـ Drop-down
        options=["تقديم طلب جديد", "متابعة الطلبات", "الاعتمادات الإدارية"],
        label_visibility="collapsed" # هذا السطر يخفي المساحة البيضاء التي يتركها العنوان الفارغ
    )
...
# تنفيذ المحتوى بناءً على اختيار القائمة المنسدلة
if choice == "تقديم طلب جديد":
    ...
    with st.container():
        ...
        signature = st.text_input("اكتب الاسم الثلاثي للإقرار")
    ...
    st.markdown("## 🔍 صفحة متابعة الطلبات")
    st.write("هنا تظهر قائمة بطلباتك السابقة وحالتها...")

elif choice == "الاعتمادات الإدارية":
    st.markdown("## ⚙️ قسم الاعتمادات")
    st.write("هذا القسم خاص بالمسؤولين فقط لاعتماد الطلبات...")
